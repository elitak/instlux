...
#undef BootCodeSizeLow
		...
		if (is_isolinux)
		{
			...
			*(unsigned long*)0x11000C = map_start_sector;
			*(unsigned long*)0x110010 = chainloader_load_length;

			old_errnum = 0; /* init to calculate the checksum */
			*(unsigned long*)(0x110000 + chainloader_load_length) = 0;

			for (p = 0; p < (chainloader_load_length - 0x40 + 3) / 4; p++)
			{
				old_errnum += *(long *)(0x110040 + (p << 2));
			}

			*(unsigned long*)0x110014 = old_errnum; /* checksum */
		}
		...
		if ((chain_edx & 0x80) == 0)
		{
		  ...
		  if (! probe_bpb((struct master_and_dos_boot_sector *)0x110000))
			...
			{
			    if (probe_mbr ((struct master_and_dos_boot_sector *)0x110000, 0, ((chainloader_load_length + 511) >> 9), 0))
			    {
	...
	if ((boot_drive & 0x80) == 0)
	{
	  ...
	  if (! probe_bpb((struct master_and_dos_boot_sector *)BOOTSEC_LOCATION))
	    ...
	    {
	      if (probe_mbr ((struct master_and_dos_boot_sector *)BOOTSEC_LOCATION, 0, 1, 0))
	      {
		errnum = ERR_HD_VOL_START_0;
	  	break;
	      }
...
//grub_printf("current_drive=0x%x, current_partition=0x%x(part_start=0x%x)\n", current_drive, current_partition, part_start);
  ...
  if (((*(long long *)BOOTSEC_LOCATION) & 0xFFFFFFFFFFFFLL) == 0x7C40EAFALL && filemax > 0x2000 && filemax < 0x20000 &&
       (*((unsigned short *) (BOOTSEC_LOCATION + 0x40)) == 0x892E) &&
       (*((unsigned short *) (BOOTSEC_LOCATION + 0x45)) == 0x8C2E) &&
       (*((unsigned long *) (BOOTSEC_LOCATION + 0x4A)) == 0xD08EC031) &&
       (*((unsigned long *) (BOOTSEC_LOCATION + 0x4E)) == 0x8E7BD4BC)
     )
    {
...
//			 &ext_offset, mbr))
      ...
      if (part == current_partition)
	{
	  ...
	  if (hidden == -1)	/* status only */
		return (PC_SLICE_TYPE (mbr, entry1) & PC_SLICE_TYPE_HIDDEN_FLAG);
...
{
  return make_saved_active((grub_memcmp (arg, "--status", 8) == 0));
}
...
//	      else
		if (mem > 0)//force the base value
		{
		  ...
		  if (bios_drive_map[i].to_drive == 0xFF)
	            {
...
delete_drive_map_slot:
  ...
  for (j = DRIVE_MAP_SIZE - 1; j > i ; j--)
    {
      if (bios_drive_map[j].to_drive == 0xFF)
        {
	  if (mem == -1)
...
real_root_func (char *arg, int attempt_mnt)
{
  ...
  if (! *arg)
	...
	print_root_device ();
      errnum = 0;
      /* return TRUE for harddrive, and FALSE for floppy */
      return (saved_drive & 0x80);
    }
  
  if (grub_memcmp (arg, "endpart", 7) == 0)
    {
	unsigned long part = 0xFFFFFF;
	unsigned long start, len, offset, ext_offset1;
	unsigned long type, entry1;
