...
init_page (void)
{
  ...
  grub_printf ("GRUB4DOS " GRUB4DOS_VERSION /* ", GNU GRUB %s" */ ", Memory: %dK / %dM, CodeEnd: 0x%X\n",
	  /* version_string, */ mbi.mem_lower, mbi.mem_upper / 1024, get_code_end());
}
...
//  return ! errnum;
  ...
  if (! addr /* (addr < (unsigned long)(RAW_ADDR (0x1000))) */
	  ...
	  && (unsigned long)(RAW_ADDR (mbi.mem_lower * 1024)) < (addr + len)) */
      || (addr >= (unsigned long)(RAW_ADDR (0x100000))
	  && (unsigned long)(RAW_ADDR (mbi.mem_upper * 1024)) < ((addr - 0x100000) + len)))
    errnum = ERR_WONT_FIT;
