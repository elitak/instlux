...
/* Turn on the active flag for the partition SAVED_PARTITION in the
   ...
   non-zero.  */
int
make_saved_active (int status_only)
{
#ifdef GRUB_UTIL
//  char mbr[512];
#endif
  ...
  if (saved_drive & 0x80)
      ...
      if (IS_PC_SLICE_TYPE_EXTENDED (PC_SLICE_TYPE (mbr, part)))
	{
      ...
      if (status_only)
	  return (PC_SLICE_FLAG (mbr, part) == PC_SLICE_FLAG_BOOTABLE);
...
grub_open (char *filename)
{
  ...
  if (*filename != '/')
...
#ifndef NO_BLOCK_FILES
      char *ptr = filename;
      unsigned long tmp, list_addr = BLK_BLKLIST_START;
      filemax = 0;
      ...
      while (list_addr < BLK_MAX_ADDR)
	{
	  ...
	  if (!safe_parse_maxint (&ptr, (int *)(void *)&tmp)
	      || tmp == 0
  ...
  if (block_file)
      ...
      while (len && !errnum)
	{
	  ...
	  while (filepos > BLK_CUR_FILEPOS)
	    {
	      if ((filepos - (BLK_CUR_FILEPOS & ~(buf_geom.sector_size - 1)))
		  >= buf_geom.sector_size)
		{
		  BLK_CUR_FILEPOS += buf_geom.sector_size;
		  BLK_CUR_BLKNUM++;
...
//printf("BLK_CUR_BLKLIST=%X, BLK_BLKSTART (BLK_CUR_BLKLIST)=%X, BLK_CUR_BLKNUM=%X, off=%X, size=%X\n", BLK_CUR_BLKLIST, BLK_BLKSTART (BLK_CUR_BLKLIST), BLK_CUR_BLKNUM, off, size);
